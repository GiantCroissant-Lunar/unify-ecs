...
namespace UnifyEcs.Sample.FrifloGame
{
    ...
    {
        [Query]
        private void Move(ref SamplePosition pos)
        {
            pos.X += 1f;
        }
    }
    ...
    public partial class SpawnWithVelocitySystem
    {
        ...
        private void Spawn(ref SamplePosition pos)
        {
            if (pos.X < 5f)
            {
                var e = Commands.CreateEntity();
                Commands.Add(e, new SamplePosition { X = pos.X + 20f, Y = pos.Y });
    ...
    public partial class SpawnSystem
    {
        ...
        private void Spawn(ref SamplePosition pos)
        {
            if (pos.X < 10f)
            {
                Commands.CreateEntity(new SamplePosition { X = pos.X + 10f, Y = pos.Y });
            }
        }
    }
    ...
    public partial class KillIfTooFarSystem
    {
        ...
        private void Kill(Entity entity, ref SamplePosition pos)
        {
            if (pos.X > 100f)
            {
    ...
    {
        [Query]
        private void Cache(ref SamplePosition pos, ref Perception perception)
        {
